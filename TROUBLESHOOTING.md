...
DB_URL = os.getenv("DATABASE_URL")  # Set in .env or environment variables
if not DB_URL:
   raise RuntimeError("DATABASE_URL not set. Add it to .env or environment variables before testing the c<<<<<<< HEAD
...
DB_URL = os.getenv("DATABASE_URL")  # Set in .env or environment variables
if not DB_URL:
   raise RuntimeError("DATABASE_URL not set. Add it to .env or environment variables before testing the connection.")
engine = create_engine(DB_URL)

with engine.connect() as conn:
   result = conn.execute(text("SELECT 1"))
   print("✓ Connection successful!")
>>>>>>> db5727d (Remove hardcoded DB credentials; load DATABASE_URL from env; add .env and .gitignore)
ls
